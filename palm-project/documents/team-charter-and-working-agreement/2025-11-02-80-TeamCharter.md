...
| :---- | :---- | :---- | :---- | :---- | :---- | :---- |
| Full VR Support | Enable remote operation using VR headset (Meta Quest) and 2D display to multiple members of an online conference room. Integrate an improved camera (4k 60fps) and use an nvidia jetson processor to encode the images for quicker and better quality streaming.  | The following must be satisfied. 1\. Video streaming at 60FPS with a minimum 1080p video quality. 2\. Latency of less than 50ms.   | REVOBOTS staff will provide a peer review of the code created for this. It must follow the standard that REVOBOTS uses for its code and must be easily readable. The expectation is that we need to produce an improved version of the current VR system.  | Server access to the streamed video MUST be secure and traffic encrypted. | Documentation will be expanded for improved streaming.  This must follow the REVOBOTS documentation standard. | Additional unit tests will be created in order to verify functionalities.  |
