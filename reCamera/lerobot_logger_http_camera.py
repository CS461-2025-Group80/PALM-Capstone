...
class LeRobotLogger(Node):
    ...
    def setup_camera(self, use_camera_subscription, filename):
        ...
        self.frame_count = 0
        # the FPS the MP4 will be written at
        self.fps = 30
        self.width = 640
        self.height = 480
        # the latest frame received by the HTTP camera receiver
        self.frame = None
        # whether we're currently capturing camera frames or not
        self.is_capturing_camera = True

        # start HTTP receiver for reCamera frames
        self.setup_http_camera_receiver(host="0.0.0.0", port=5000)
    ...
    def setup_http_camera_receiver(self, host="0.0.0.0", port=5000):
        node = self
                ...
                try:
                    ...
                    if image_b64:
                        ...
                        if frame is not None:
                            frame = cv2.resize(frame, (node.width, node.height))
                            node.frame = frame
                    ...
                    node.get_logger().error(f"HTTP frame decode failed: {e}")
        ...
        self.http_server = HTTPServer((host, port), FrameHandler)
        self.http_thread = threading.Thread(target=self.http_server.serve_forever, daemon=True)
        self.http_thread.start()
        self.get_logger().info(f"HTTP camera receiver listening on {host}:{port}")
